...
import com.example.mobileapp.Adapter.PopularAdapter;
import com.example.mobileapp.Model.CategoryModel;
import com.example.mobileapp.Model.ItemModel; // ✅ Use the correct model for Popular items
        ...
        myRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (!list.isEmpty()) {
                        binding.recyclerViewCatagory.setLayoutManager(
                                new GridLayoutManager(MainActivity.this, 4)
                        );
                        CategoryAdapter adapter = new CategoryAdapter(list);
                        binding.recyclerViewCatagory.setAdapter(adapter);
                    }
                }
